...
def main():
    ...
    pendulum_left = cv.imread('data/pendulum/im0.png',1)
    pendulum_left = cv.resize(pendulum_left, (int(pendulum_left.shape[1]*0.5), int(pendulum_left.shape[0]*0.5)))

    pendulum_right = cv.imread('data/pendulum/im1.png',1)
    pendulum_right = cv.resize(pendulum_right, (int(pendulum_right.shape[1]*0.5), int(pendulum_right.shape[0]*0.5)))

    list_kp1, list_kp2 = getMatchingFeature(pendulum_left, pendulum_right)
    ...
    pendulum_left_copy = copy.copy(pendulum_left)
    pendulum_left_epiline = drawlines(pendulum_left_copy, lines1, best_kp1)

    # Find epilines corresponding to points in left image (first image) and
    # drawing its lines on right image
    lines1 = cv.computeCorrespondEpilines(best_kp1.reshape(-1, 1, 2), 2, F)
    lines1 = lines1.reshape(-1, 3)
    pendulum_right_copy = copy.copy(pendulum_left)
    pendulum_right_epiline = drawlines(pendulum_right_copy, lines1, best_kp2)
    
    
    image = np.concatenate((pendulum_left_epiline,pendulum_right_epiline), axis =1)
    # cv.imshow('Rectified Epipolar Line', image)
    cv.imwrite('Epipolar.png', image)

    h1, w1,_ = pendulum_left.shape
    h2, w2,_ = pendulum_right.shape
    _, H1, H2 = cv.stereoRectifyUncalibrated( np.float32(best_kp1), np.float32(best_kp2), F, imgSize=(w1, h1))
    
    pendulum_left_rectified_epi = cv.warpPerspective(pendulum_left_epiline, H1, (w1, h1))
    pendulum_right_rectified_epi = cv.warpPerspective(pendulum_right_epiline, H2, (w2, h2))
    image = np.concatenate((pendulum_left_rectified_epi,pendulum_right_rectified_epi), axis =1)
    cv.imshow('Rectified Epipolar Line', image)
    cv.imwrite('Rectified_Epipolar.png', image)
    
    pendulum_left_rectified = cv.warpPerspective(pendulum_left, H1, (w1, h1))
    pendulum_right_rectified = cv.warpPerspective(pendulum_right, H2, (w2, h2))
    
    pendulum_left_rect_gray = cv.cvtColor(pendulum_left_rectified , cv.COLOR_BGR2GRAY)
    pendulum_right_rect_gray = cv.cvtColor(pendulum_right_rectified , cv.COLOR_BGR2GRAY)
    
    disparity_map = calculateDisparity(pendulum_left_rect_gray, pendulum_right_rect_gray)
